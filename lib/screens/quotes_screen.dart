...
class _QuotesScreenState extends State<QuotesScreen> {
  ...
  void _handleSelectionAction(
    ...
    switch (action) {
      ...
      case _SelectionAction.delete:
        _deleteSelected(controller, strings);
        break;
    }
  }
  ...
  Future<void> _deleteSelected(
    ...
    final approved = await showDialog<bool>(
      ...
      builder: (context) {
        return AlertDialog(
          title: Text(strings.deleteSelectedTitle(selected.length)),
          content: Text(strings.deleteSelectedWarning),
          actions: [
            TextButton(
    ...
    for (final quote in selected) {
      await _quoteRepository.deleteById(quote.id);
    }
    ...
    ScaffoldMessenger.of(context)
      ...
      ..showSnackBar(
        SnackBar(
          content: Text(strings.selectedDeleted(selected.length)),
          duration: const Duration(seconds: 2),
        ),
      );
  }
...
class _SelectionMenuItem extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final color = danger ? const Color(0xFFB84A3A) : null;
    return Row(
      children: [
